...
class Dropout(Layer):
	...
	def do_init(self, kwargs):
		...
		self.mask = np.random.random((self.batches, *input_shape), dtype=self.dtype) > self.rate
		self.param = 0
	...
	def forward(self, inp, training=True):
		if training:		# generate mask with rate probability
			...
			self.mask = self.scale * np.random.binomial(1, 1-self.rate, inp.shape).astype(self.dtype, copy=False)
